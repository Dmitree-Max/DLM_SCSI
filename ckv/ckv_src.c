			   ...
			   loff_t * ppos)
{
	...
	bytes_written =
	    get_values(device_buffer, size_in_min_buffer_left, ppos);

	bytes_send =
	    bytes_written - copy_to_user(buff, device_buffer, bytes_written);
	*ppos += bytes_send;
			    ...
			    loff_t * ppos)
{
	...
	bytes_written =
	    size_in_min_buffer_left - copy_from_user(device_buffer, buff,
						     size_in_min_buffer_left);
